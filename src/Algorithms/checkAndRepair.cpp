...
void Basic_TMesh::repair(const RepairOptions& options, RepairReport& report)
{
	if (options.join_multiple_components)
	{
		...
		while (joinClosestComponents(this))
		{
			...
			TMesh::report_progress("Num. components: %d       ", shells());
		}
		TMesh::end_progress();
		deselectTriangles();
	}

	report.removed_components = removeSmallestComponents();
	...
	if (boundaries())
	{
		...
		report.patched_holes = fillSmallBoundaries(0, true);
	}

	if (!boundaries())
	{
		report.degeneracies_before = countExactDegeneracies();
		report.intersections_before = countSelfIntersectingTriangles();
		...
		if (report.degeneracies_before > 0 || report.intersections_before > 0)
		{
			...
			if (!meshclean()) TMesh::warning("MeshFix could not fix everything.\n", report.removed_components);
			report.degeneracies_after = countExactDegeneracies();
			report.intersections_after = countSelfIntersectingTriangles();
		}
	}
